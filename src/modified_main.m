...
while not(done)
    ...
    init_ds=solv.t_curr >= solv.t_imp_des(solv.f_iter)-solv.DSpar*(solv.t_imp_des(solv.f_iter)-solv.t_imp_des(solv.f_iter-1));
    if solv.replan && init_ds
            ...
            solv.t_imp_des = plan.t_imp_des; % DCM at the end of a step 
            solv.dcm_traj_des_eos_store = plan.dcm_traj_des_eos_store; % Impact times
            solv.replan = false;
    end
...
plot(solv.dcm_traj_des_store(1,1:iter), solv.dcm_traj_des_store(2,1:iter), 'g')
xlabel("Position in space [m]", 'interpreter', 'latex', 'FontSize', 14)
ylabel("Position in space [m]", 'interpreter', 'latex', 'FontSize', 14)
